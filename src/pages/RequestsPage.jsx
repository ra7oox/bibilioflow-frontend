...
export const RequestsPage = () => {
  ...
  // États pour la notation et signalement
  ...
  const handleStatusChange = async (request, newStatus) => {
    try {
      ...
      fetchRequests();
      setNotification('Action enregistrée avec succès !');
  ...
  // ====================================================================
  // NOUVELLE FONCTIONNALITÉ : ANNULER UNE DEMANDE (EMPRUNTEUR)
  // ====================================================================
  const handleCancelRequest = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setNotification("✅ Demande annulée avec succès !");
        fetchRequests();
        setTimeout(() => setNotification(null), 3000);
      }
    } catch (error) {
  ...
  // ====================================================================
  // TÂCHE 4 : GÉNÉRER LE MESSAGE DE RAPPEL (contenu automatique)
  // ====================================================================
  const generateReminderMessage = (request) => {
    ...
    let message = `Rappel concernant le livre « ${request.bookTitle} ».`;
    
    if (diffDays < 0) {
      message += ` ⚠️ Attention : le livre aurait dû être rendu il y a ${Math.abs(diffDays)} jour(s). Merci de le restituer dès que possible.`;
    } else if (diffDays === 0) {
      message += ` 📅 Le livre doit être rendu aujourd'hui (${request.returnDate}).`;
    } else {
      message += ` 📅 Date de retour prévue : ${request.returnDate} (dans ${diffDays} jour(s)).`;
    }
  ...
  // ====================================================================
  // TÂCHE 4 : ENVOYER LE RAPPEL À L'EMPRUNTEUR (notification + email)
  // ====================================================================
  const handleSendReminder = async (request) => {
    ...
    try {
      console.log('📧 Envoi du rappel à:', request.requesterName);
      ...
      const sendResponse = await fetch(`https://biblioflow-production-022b.up.railway.app/notifications/send`, {
        ...
        body: JSON.stringify({
          ...
          subject: `📚 Rappel : Retour du livre "${request.bookTitle}"`,
          message: autoGeneratedMessage,
      ...
      if (!sendResponse.ok) {
        throw new Error("Échec de l'envoi du rappel");
      }

      const sendData = await sendResponse.json();
      console.log('✅ Rappel envoyé avec succès:', sendData);
      ...
      if (response.ok) {
        setNotification(`🔔 Rappel envoyé avec succès à ${request.requesterName} par notification et email !`);
        fetchRequests();
        setTimeout(() => setNotification(null), 3000);
      }
    } catch (error) {
      console.error("Erreur lors de l'envoi du rappel :", error);
      setNotification("❌ Erreur lors de l'envoi du rappel. Réessayez.");
      setTimeout(() => setNotification(null), 3000);
    }
  };
  ...
  const handleConfirmReturn = async (req) => {
    try {
      ...
      setNotification("✅ Livre récupéré ! L'échange est clos.");
      fetchRequests();
      setTimeout(() => setNotification(null), 3000);
    } catch (error) {
  ...
  // ====================================================================
  // TÂCHE 1 : ENREGISTRER LA NOTE ET LE COMMENTAIRE
  // ====================================================================
  const submitRating = async () => {
  // Validation de la notation
  if (rating === 0) {
    setNotification("⚠️ Veuillez sélectionner au moins une étoile");
    setTimeout(() => setNotification(null), 3000);
    return;
  }

  try {
    // Préparation des données de notation
    const ratingData = {
      rating: rating,
      comment: comment,
    ...
    if (response.ok) {
      console.log("✅ Notation enregistrée avec succès", ratingData);
        // ============================================================
        // TÂCHE 2 : METTRE À JOUR LA NOTE MOYENNE DE L'UTILISATEUR
        // ============================================================
        try {
          // Déterminer qui doit recevoir cette note
          ...
          console.log(`✅ Note moyenne mise à jour pour l'utilisateur ${ratedUserId}:`, ratingStats.averageRating);
        } catch (error) {
          console.warn("⚠️ Note enregistrée mais moyenne non calculée:", error);
        }
        ...
        setNotification("⭐ Merci pour votre avis ! Il a été enregistré.");
        setTimeout(() => setNotification(null), 3000);
      ...
      setNotification("❌ Erreur lors de l'enregistrement. Réessayez.");
      setTimeout(() => setNotification(null), 3000);
    }
  };

  // ====================================================================
  // TÂCHE 2 & 3 : ENREGISTRER LE SIGNALEMENT + NOTIFIER L'ADMIN
  // ====================================================================
  const handleSubmitReport = async () => {
    if (!reportDescription.trim()) {
      setNotification("⚠️ Veuillez décrire l'incident");
      setTimeout(() => setNotification(null), 3000);
      return;
    }
    ...
    try {
      ...
      console.log("✅ Signalement enregistré:", savedReport);
      
      // Le backend a automatiquement notifié les admins
      if (savedReport.adminsNotified) {
        console.log(`✅ ${savedReport.adminsNotified} administrateur(s) notifié(s)`);
      }
      ...
      if (!updateResponse.ok) {
        console.error("❌ Erreur mise à jour de la demande");
      }
      ...
      setNotification(`✅ Signalement enregistré et ${savedReport.adminsNotified || 0} admin(s) notifié(s) !`);
      setTimeout(() => setNotification(null), 3000);
      
      fetchRequests();

    } catch (error) {
      console.error("❌ Erreur complète lors de l'enregistrement du signalement:", error);
      setNotification(`❌ Erreur: ${error.message}. Vérifiez que le serveur est démarré.`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-12 px-4 relative transition-colors duration-200">
      ...
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-black text-gray-900 dark:text-gray-100 mb-8 tracking-tight italic uppercase transition-colors">Gestion des Échanges</h1>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden transition-colors">
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-100 dark:border-gray-600 transition-colors">
              <tr>
                <th className="p-5 font-bold text-gray-400 dark:text-gray-300 uppercase text-xs tracking-widest">Livre & Échéance</th>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
              {requests.map((req) => (
                <tr key={req.id} className="hover:bg-indigo-50/20 dark:hover:bg-indigo-900/10 transition-colors">
                  <td className="p-5">
                    <span className="font-bold text-gray-900 dark:text-gray-100 block">📖 {req.bookTitle}</span>
                    {req.status === 'accepted' && req.returnDate && (
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase ${new Date(req.returnDate) < new Date() ? 'bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-400' : 'bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400'}`}>
                        {new Date(req.returnDate) < new Date() ? '⚠️ Retard' : 'Retour'} : {req.returnDate}
                      </span>
                    )}
                    {req.hasReport && (
                      <span className={`ml-2 text-[9px] font-black px-2 py-0.5 rounded uppercase ${
                        ...
                        🚨 {req.reportStatus === 'en_attente' ? 'Signalé' : 
                            req.reportStatus === 'traite' ? 'En cours' : 'Résolu'}
                      </span>
                    )}
                  </td>
                  ...
                  <td className="p-5 text-right flex justify-end gap-3 items-center">
                    ...
                    {currentUser.role === 'preteur' && req.status === 'pending' ? (
                      ...
                      req.status === 'accepted' && (
                        req.isFinished ? (
                          !req.hasBeenRated && (
                            <button onClick={() => setSelectedRequestForRating(req)} className="bg-amber-400 dark:bg-amber-500 text-white px-3 py-1.5 rounded-xl text-xs font-bold shadow-lg shadow-amber-100 dark:shadow-amber-900/30">⭐ Noter</button>
                          )
                        ) : (
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-2xl transition-colors">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 font-medium">
              Êtes-vous sûr de vouloir annuler votre demande pour "<span className="font-bold text-gray-900 dark:text-gray-100">{showCancelModal.bookTitle}</span>" ?
            </p>
     ...
     <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
       <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-2xl transition-colors">
         ...
         <p className="text-xs text-gray-500 dark:text-gray-400 mb-4 font-medium">
           Votre signalement sera enregistré et les administrateurs seront notifiés automatiquement.
         </p>
         ...
         >
           <option value="non_rendu">📕 Livre non restitué</option>
           <option value="abime">💔 Livre détérioré</option>
           <option value="absence">👻 Rendez-vous manqué</option>
           <option value="comportement">⚠️ Comportement inapproprié</option>
         </select>

         <textarea
           value={reportDescription}
           onChange={(e) => setReportDescription(e.target.value)}
           placeholder="Décrivez brièvement la situation... (obligatoire)"
      ...
      {selectedRequestForRating && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 max-w-md w-full shadow-2xl transition-colors">
            <h2 className="text-2xl font-black text-gray-900 dark:text-gray-100 mb-2">Avis sur l'échange</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">
              Votre note sera enregistrée et le profil de l'utilisateur sera mis à jour
            </p>
